...
describe('test signP2PKProof', () => {
	...
	test('sign with 02-prepended Nostr key', async () => {
		...
		const PRIVKEY2_BYTES = hexToBytes(PRIVKEY2);
		const PUBKEY2 = '02' + bytesToHex(schnorr.getPublicKey(PRIVKEY2_BYTES)); // Prepended x-only pubkey
		const PUBKEY3 = bytesToHex(getPubKeyFromPrivKey(PRIVKEY2_BYTES)); // full 33-byte pubkey
