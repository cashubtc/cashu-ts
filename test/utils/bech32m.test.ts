...
import { decodeBech32mToBytes } from '../../src/utils/bech32m';
import { decodeTLV } from '../../src/utils/tlv';
...
describe('NUT-26 Test Vectors', () => {
	...
	describe('Payment Request with NUT-10 Locking', () => {
		...
		test('decodeTLV parses payment request (nut10 not yet implemented)', () => {
			const bytes = decodeBech32mToBytes(encoded);
			const decoded = decodeTLV(bytes);
			...
			expect(decoded.amount).toBe(BigInt(expected.a));
			expect(decoded.mints).toEqual(expected.m);
			// NUT-10 parsing not yet implemented
		});
	});

	describe('HTTP POST Transport (kind=2)', () => {
		const encoded =
	...
	describe('In-Band Transport (kind=0)', () => {
		const encoded =
		...
		test('decodeTLV parses in-band transport', () => {
			const bytes = decodeBech32mToBytes(encoded);
			const decoded = decodeTLV(bytes);
	...
	describe('NUT-10 HTLC Locking (kind=1)', () => {
		...
		test('decodeTLV parses HTLC locking (nut10 not yet implemented)', () => {
			const bytes = decodeBech32mToBytes(encoded);
			const decoded = decodeTLV(bytes);
