...
describe('mint api', () => {
	...
	test('mint and melt p2pk', async () => {
		...
		const proofs = await wallet.mintTokens(3000, mintRequest.quote, {pubkey: bytesToHex(pubKeyBob)});

		const meltRequest = await wallet.createMeltQuote(externalInvoice);
		const fee = meltRequest.fee_reserve;
		expect(fee).toBeGreaterThan(0);
		const response = await wallet.meltTokens(meltRequest, proofs.proofs, {privkey: bytesToHex(privKeyBob)});
		expect(response).toBeDefined();
