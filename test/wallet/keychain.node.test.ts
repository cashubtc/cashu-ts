...
describe('KeyChain initialization', () => {
  ...
  test('fromCache rejects a cache recorded for a different mint', async () => {
    const chain = new KeyChain(mint, unit);
    await chain.init();
    const foreign = { ...chain.cache, mintUrl: 'https://other-mint.example' };
    expect(() => KeyChain.fromCache(mint, unit, foreign)).toThrow(/different mint/);
  });

  test('fromCache names the missing mint URL on a cache that has none', async () => {
    const chain = new KeyChain(mint, unit);
    await chain.init();
    const { mintUrl: _dropped, ...nameless } = chain.cache;
    expect(() => KeyChain.fromCache(mint, unit, nameless as KeyChainCache)).toThrow(
      /missing its mint URL/,
    );
  });
  ...
  test('fromCache accepts a cache whose mint URL differs only in normalization', async () => {
    ...
    const restored = KeyChain.fromCache(mint, unit, unnormalized);
...
describe('KeyChain getters', () => {
  ...
  test('treats an explicit empty keyset ID as unknown rather than the default', () => {
    expect(() => keyChain.getKeyset('')).toThrow("Keyset '' not found");
  });
...
describe('KeyChain refuses keyset id version bytes this build cannot spend', () => {
  ...
  test('auto-selection skips it in favour of a supported keyset', async () => {
    ...
    expect(chain.getCheapestKeyset().id).toBe(supported.meta.id);
  });

  test('auto-selection names the version gap when every active keyset is too new', async () => {
    const chain = await initChainWith([makeFutureKeyset(1).keys]);
  ...
  test('getKeyset stays a plain lookup, so filters and fee sums do not throw', async () => {
    ...
    // here would abort the whole operation over one keyset the caller never asked for.
    expect(() => chain.getAllKeysetIds().map((id) => chain.getKeyset(id).unit)).not.toThrow();
    expect(chain.getKeyset(future.meta.id).version).toBe(unsupportedVersion);
  });

  test('ensureKeysetKeys refuses it instead of reporting a verification failure', async () => {
    const future = makeFutureKeyset(1);
