...
describe('test deriveKeysetId', () => {
	test('testing deriveKeysetId', async () => {
		const keys = {
		...
		const keysetId = 'I2yN+iRYfkzT';

		const result = await utils.deriveKeysetId(keys);
		expect(result).toBe(keysetId);
	});
});
