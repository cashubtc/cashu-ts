...
- **The scan yields between chunks of local work.** Deriving, hashing and blinding a BLS batch is about a tenth of a second of CPU, so the scan hands the thread back every few dozen counters; a UI stays responsive and `signal` is honoured there as well as between requests. `onProgress` fires after each wave with the next counter to scan and the proofs found so far.
