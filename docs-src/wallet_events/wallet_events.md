...
**Keepalive:** a socket that dies without a close frame (a proxy or NAT timing out an idle connection) looks exactly like a mint with nothing to report. Pass `wsKeepaliveMs` to the `Wallet` or `Mint` constructor and the connection sends a small JSON-RPC probe at that interval; a probe the mint does not answer within the interval (10 s at most) drops the socket as an abnormal close, so every subscription's error callback fires and a `pollMs` watch falls back to polling. Off by default. Both cdk and Nutshell answer the probe in well under a second, so an interval of 30 s or more is plenty.
