...
export class OIDCAuth {
  ...
  /**
   * Assert a discovery endpoint is http(s); warn when discovery was https and the endpoint is not.
   *
   * @remarks
   * Cashu-ts v5 rejects the https-to-http case instead of warning.
   */
  private assertEndpoint(url: string, label: string): void {
    ...
    if (this.secureDiscovery && protocol !== 'https:') {
      this.logger.warn(`OIDCAuth: ${label} is not https although discovery is; v5 rejects this`, {
        label,
      });
    }
  }
