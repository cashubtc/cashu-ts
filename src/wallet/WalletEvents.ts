...
function guarded<T>(normalize: (p: T) => T, err: (e: Error) => void): (p: T) => T | undefined {
  return (p) => {
    try {
      return normalize(p);
    } catch (e) {
...
export class WalletEvents {
  ...
  async mintQuoteUpdates(
    ...
    return this._watch<MintQuoteBolt11Response>(
      { kind: 'bolt11_mint_quote', filters: uniq, decode: guarded(normalizeMintQuoteUpdate, err) },
  ...
  async meltQuoteUpdates(
    ...
    return this._watch<MeltQuoteBolt11Response>(
      { kind: 'bolt11_melt_quote', filters: uniq, decode: guarded(normalizeMeltQuoteUpdate, err) },
      {
