...
| `restore_example.ts`      | `DEV=1 INPUT_FEE_PPK=100 RATE_LIMIT_PM=5000 make nutshell-up`                     | Churns a wallet, then recovers it from seed three ways and reports what each costs: `filterSpent: false` (restore every issued counter), the default (state check first, restore only what is live), and the default at a smaller `batchSize`. Knobs: `CHURN_ROUNDS`, `SPEND_FRAC`, `PIN_MID`.                                                                                                                                             |
